...
def generateDataset(
    ...
    for e in range(pSize):
        ...
        pRel = sc.duple(oTerm, exampleTerm, True, generation, region)
        pRel.pick = pick
        pRel.place = place
        ...
        if useEnvironment and len(nDataSet) < nSize/3:
            ...
            if eid.pick[0] != pick[0] or eid.pick[1] != pick[1] or eid.place[0] != place[0] or eid.place[1] != place[1]:
                ...
                nRel = sc.duple(noTerm, exampleTerm, False, generation, region)
    ...
    while len(nDataSet) < nSize:
        ...
        if abs((pick1[0] - pick2[0])**2 + (pick1[1] - pick2[1])**2) > RADIO**2:
            nRel = sc.duple(oterm2, exampleTerm1, False, generation, region)
            nDataSet.append(nRel)
...
def realValuedOutputBatchLearning(
    ...
    for i in range(MAX_ITER):
        ...
        if True:
            ...
            testSpace = sc.termSpace()
            for rel in testSet:
                rel.wL = testSpace.add(rel.L)
                rel.wH = testSpace.add(rel.R)
                testConsts |= rel.L
                testConsts |= rel.R
            las = sc.calculateLowerAtomicSegment(target, testConsts, True)
            ...
            if fullTest:
                ...
                (
                    ...
                    misses,
                ) = ql.evaluateUsingUnionModel(testSet, 1, region)
                print(">")
...
def trainModel(window, params):
    ...
    eid = sc.exampleInterpretationData()
    eid.window = window
...
def testModel(window,  name, exampleGeneratorFunction, constants):
    ...
    cmanager, atomization = ql.loadAtomizationFromFileUsingBitarrays(name)  #
    ...
    if True:
        ...
        for s in [0, 1, 2, 3]:
            ...
            fields[s] = cFromVal
        testConsts = sc.CSegment(constants)

        # associate atoms to constents
        las = sc.calculateLowerAtomicSegment(atomization, testConsts, True)
