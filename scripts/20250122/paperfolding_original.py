# -*- coding: utf-8 -*-import cProfil


# use to print in colores if Windows is used
# import termcolor

import aml_engine as aml
from aml_engine import amlMaster as mt
from aml_engine.config import *
    ...
    for i in range(MAX_ITER):
        ...
        if True and i % 25 == 0:
            ...
            aml.saveAtomizationOnFileUsingBitarrays(   
                ...
                cmanager,
                "ATOMIZATIONS/PAPERFOLD_" + specs + str(i),
            )
...
def trainModel(window, params):
    ...
    if PARALLEL_STRATEGY == ParallelStrategy.Nothing:
        batchLearner = aml.batchLearner(model)
        batchLearner.params.storePositives = True
    elif PARALLEL_STRATEGY in (ParallelStrategy.File, ParallelStrategy.Shared, ParallelStrategy.DDS):  # fmt:skip
        # Distributed
        batchLearner = mt.master(model)
        batchLearner.expectedMaxNumberOfNodes = 5
    else:
        raise ValueError("PARALLEL_STRATEGY must be a valid strategy")
...
if __name__ == "__main__":
    ...
    if False:
        ...
        name = "ATOMIZATIONS/PAPERFOLD_200"
